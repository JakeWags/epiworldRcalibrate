---
title: "Comparing BiLSTM and ABC Calibration for SIR Models"
subtitle: "Utah COVID-19 Case Prediction with Uncertainty Quantification"
author: "Sima Najafzadehkhoei"
date: today
format:
  html:
    toc: true
    toc-depth: 3
    code-fold: show
    code-tools: true
    theme: cosmo
    embed-resources: true
editor: visual
---

## Introduction

This analysis compares two approaches to epidemic model calibration:

1.  **BiLSTM (Deep Learning)**: Fast neural network-based parameter estimation
2.  **ABC (Approximate Bayesian Computation)**: Simulation-based Bayesian inference

Both methods calibrate SIR model parameters from observed COVID-19 incidence data, but differ dramatically in computational cost and uncertainty quantification.

**Important Note**: Both methods use the **known recovery rate** (1/7, corresponding to a 7-day infectious period) as a fixed parameter, calibrating only contact rate and transmission probability. This ensures a fair comparison and leverages domain knowledge about COVID-19's infectious period.

### Workflow Overview

1.  Download recent COVID-19 case data from Utah (61 days)
2.  **BiLSTM calibration**: Estimate parameters using trained neural network
3.  **ABC calibration**: Load pre-computed parameters from package data
4.  Run 2,000 stochastic SIR simulations for each method
5.  Compare predictions, uncertainty, and computational time

## Setup
```{r setup, message=FALSE, warning=FALSE}
suppressPackageStartupMessages({
  library(tidyverse)
  library(ggplot2)
  library(epiworldR)
  library(epiworldRcalibrate)
  library(tictoc)
  library(data.table)
  library(knitr)
})
```

## Data Acquisition

### Load Packaged Data
```{r data-load}
data("utah_covid_data")
data("abc_calibration_params")

# Preview the COVID data
head(utah_covid_data)
summary(utah_covid_data)
```

### Load Recent Case Data
```{r load-data}
get_covid_data <- function(n_days) {
  
  # Use packaged data
  covid_data <- utah_covid_data
  
  # Filter to last n_days
  last_date <- max(covid_data$Date, na.rm = TRUE)
  covid_data <- subset(covid_data, Date > (last_date - n_days)) %>% 
    dplyr::arrange(Date)
  
  stopifnot("Daily.Cases" %in% names(covid_data))
  covid_data
}

n_days <- 61
covid_data <- get_covid_data(n_days)
incidence <- covid_data$Daily.Cases
incidence_vec <- incidence

stopifnot(length(incidence) == n_days)

cat("Date range:", as.character(min(covid_data$Date)), "to", 
    as.character(max(covid_data$Date)), "\n")
cat("Total cases observed:", sum(incidence), "\n")
cat("Mean daily cases:", round(mean(incidence), 1), "\n")
```

### Configuration
```{r config}
N <- 20000     # Population size
recov <- 1 / 7  # Recovery rate (KNOWN: 7-day infectious period)
model_ndays <- 60  # Simulation days

cat("\n** Using KNOWN recovery rate:", recov, "**\n")
cat("Both BiLSTM and ABC calibrate only: contact_rate and transmission_prob\n")
```

## Method 1: BiLSTM Calibration

### Parameter Estimation
```{r bilstm-calibration}
cat("=== BiLSTM Calibration ===\n")

tic("BiLSTM calibration")
lstm_predictions <- calibrate_sir(
  daily_cases = incidence_vec,
  population_size = N,
  recovery_rate = recov  # Fixed/known parameter
)
bilstm_time <- toc()

cat("\nCalibrated Parameters:\n")
print(lstm_predictions)
cat("\nNote: Recovery rate was FIXED at", recov, "(not calibrated)\n")
```

### Stochastic Simulations (BiLSTM)
```{r bilstm-simulations}
cat("\n=== Running 2,000 Stochastic Simulations (BiLSTM) ===\n")

init_infected <- incidence[1]
prev <- init_infected / N

bilstm_model <- ModelSIRCONN(
  name              = "BiLSTM SIR",
  n                 = N,
  prevalence        = prev,
  contact_rate      = lstm_predictions[["crate"]],
  transmission_rate = lstm_predictions[["ptran"]],
  recovery_rate     = recov  # Using known value
)

saver_bilstm <- make_saver("transition")

tic("BiLSTM simulations")
run_multiple(
  bilstm_model,
  ndays    = model_ndays,
  nsims    = 2000,
  saver    = saver_bilstm,
  nthreads = 8
)
bilstm_sim_time <- toc()

bilstm_results <- run_multiple_get_results(
  bilstm_model,
  nthreads = 8,
  freader = data.table::fread
)
```

### Extract BiLSTM Confidence Intervals
```{r bilstm-ci}
bilstm_transitions <- bilstm_results$transition %>%
  filter(from == "Susceptible", to == "Infected") %>%
  arrange(sim_num, date)

bilstm_quantiles <- bilstm_transitions %>%
  group_by(date) %>%
  summarize(
    lower_ci = quantile(counts, 0.025),
    upper_ci = quantile(counts, 0.975),
    median   = quantile(counts, 0.5),
    mean     = mean(counts),
    .groups  = "drop"
  )
```

## Method 2: ABC Calibration

### Load Pre-Calibrated ABC Results
```{r abc-load-results}
cat("\n=== ABC Calibration (Pre-computed) ===\n")

# Load the calibration data
data("abc_calibration_params")

# Check if recovery rate was fixed during calibration
recovery_was_fixed <- abc_calibration_params$recovery_rate_fixed
n_calibrated <- abc_calibration_params$n_calibrated_params

# Extract calibrated parameters
abc_crate <- abc_calibration_params$contact_rate
abc_recov <- abc_calibration_params$recovery_rate  # This is the known value
abc_ptran <- abc_calibration_params$transmission_prob
abc_R0 <- abc_calibration_params$R0

# Extract confidence intervals (only for calibrated parameters)
abc_crate_lower <- abc_calibration_params$contact_rate_ci["lower"]
abc_crate_upper <- abc_calibration_params$contact_rate_ci["upper"]
abc_ptran_lower <- abc_calibration_params$transmission_prob_ci["lower"]
abc_ptran_upper <- abc_calibration_params$transmission_prob_ci["upper"]

# Get timing information
abc_time_minutes <- abc_calibration_params$calibration_time_seconds / 60

# Get configuration
N_abc <- 20000

n_samples_abc <- abc_calibration_params$n_samples
burnin_abc <- abc_calibration_params$burnin

cat("ABC Configuration:\n")
cat("Population size:", N_abc, "\n")
cat("MCMC samples:", n_samples_abc, "(", burnin_abc, "burn-in)\n")
cat("Epsilon:", round(abc_calibration_params$epsilon, 2), "\n")
cat("Parameters calibrated:", n_calibrated, "(contact_rate, transmission_prob)\n")
cat("Parameters fixed: 1 (recovery_rate =", abc_recov, ")\n")
cat("Original computation time:", round(abc_time_minutes, 2), "minutes\n\n")

cat("Calibrated Parameters (Median with 95% CI):\n")
cat("Contact Rate:      ", sprintf("%.4f", abc_crate), 
    " [", sprintf("%.4f", abc_crate_lower), ", ", sprintf("%.4f", abc_crate_upper), "]\n", sep = "")
cat("Recovery Rate:     ", sprintf("%.4f", abc_recov), " (FIXED/KNOWN)\n", sep = "")
cat("Transmission Prob: ", sprintf("%.4f", abc_ptran), 
    " [", sprintf("%.4f", abc_ptran_lower), ", ", sprintf("%.4f", abc_ptran_upper), "]\n", sep = "")
cat("R0:                ", sprintf("%.4f", abc_R0), "\n", sep = "")
```

### ABC Posterior Distributions
```{r abc-posterior, fig.width=10, fig.height=4}
# Extract posterior samples (only 2 parameters were calibrated)
posterior <- abc_calibration_params$posterior_samples

# Calculate R0 from posterior samples
# Note: posterior has only 2 columns now (contact_rate, transmission_prob)
R0_samples <- (posterior[, 1] * posterior[, 2]) / abc_recov

# Create data frame for plotting
posterior_df <- data.frame(
  contact_rate = posterior[, 1],
  transmission_prob = posterior[, 2],
  R0 = R0_samples
)

# Reshape for faceted plot
posterior_long <- posterior_df %>%
  pivot_longer(cols = everything(), names_to = "parameter", values_to = "value") %>%
  mutate(parameter = factor(parameter, 
    levels = c("contact_rate", "transmission_prob", "R0"),
    labels = c("Contact Rate", "Transmission Prob", "R0")
  ))

# Plot posterior distributions
ggplot(posterior_long, aes(x = value)) +
  geom_histogram(bins = 30, fill = "#2E86DE", alpha = 0.7, color = "white") +
  facet_wrap(~parameter, scales = "free", ncol = 3) +
  labs(
    title = "ABC Posterior Distributions",
    subtitle = paste0("Based on ", nrow(posterior), " MCMC samples (post burn-in). Recovery rate fixed at ", round(abc_recov, 4)),
    x = "Parameter Value",
    y = "Frequency"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    plot.title = element_text(face = "bold", hjust = 0.5),
    plot.subtitle = element_text(hjust = 0.5, size = 10),
    strip.text = element_text(face = "bold"),
    panel.grid.minor = element_blank()
  )
```

### Stochastic Simulations (ABC)
```{r abc-simulations}
cat("\n=== Running 2,000 Stochastic Simulations (ABC) ===\n")

initial_infected <- incidence_vec[1]
initial_prevalence <- initial_infected / N

abc_model <- ModelSIRCONN(
  name              = "ABC SIR",
  n                 = N,
  prevalence        = initial_prevalence,
  contact_rate      = abc_crate,
  transmission_rate = abc_ptran,
  recovery_rate     = recov  # Using known value (same as BiLSTM)
)

saver_abc <- make_saver("transition")

tic("ABC simulations")
run_multiple(
  abc_model,
  ndays    = model_ndays,
  nsims    = 2000,
  saver    = saver_abc,
  nthreads = 8
)
abc_sim_time <- toc()

abc_sim_results <- run_multiple_get_results(
  abc_model,
  nthreads = 8,
  freader = data.table::fread
)
```

### Extract ABC Confidence Intervals
```{r abc-ci}
abc_transitions <- abc_sim_results$transition %>%
  filter(from == "Susceptible", to == "Infected") %>%
  arrange(sim_num, date)

abc_quantiles <- abc_transitions %>%
  group_by(date) %>%
  summarize(
    lower_ci = quantile(counts, 0.025),
    upper_ci = quantile(counts, 0.975),
    median   = quantile(counts, 0.5),
    mean     = mean(counts),
    .groups  = "drop"
  )
```

## Comparison

### Timing Summary
```{r timing-summary}
bilstm_total_mins <- (bilstm_time$toc - bilstm_time$tic) / 60
speedup <- abc_time_minutes / bilstm_total_mins

cat("=== Computational Time Comparison ===\n\n")
cat("BiLSTM calibration:", round(bilstm_total_mins, 2), "minutes\n")
cat("ABC calibration:", round(abc_time_minutes, 2), "minutes (pre-computed)\n")
cat("\nSpeedup: BiLSTM is", round(speedup, 1), "x FASTER than ABC\n")
```

### Parameter Comparison
```{r param-comparison}
comparison_df <- data.frame(
  Parameter = c("Contact Rate", "Recovery Rate", "Trans. Prob", "R0"),
  BiLSTM = c(
    lstm_predictions[["crate"]],
    recov,
    lstm_predictions[["ptran"]],
    lstm_predictions[["R0"]]
  ),
  ABC = c(abc_crate, abc_recov, abc_ptran, abc_R0),
  Status = c("Calibrated", "Fixed (Known)", "Calibrated", "Derived"),
  Difference = c(
    lstm_predictions[["crate"]] - abc_crate,
    0,  # No difference - both use same known value
    lstm_predictions[["ptran"]] - abc_ptran,
    lstm_predictions[["R0"]] - abc_R0
  ),
  Pct_Diff = c(
    (lstm_predictions[["crate"]] - abc_crate) / abc_crate * 100,
    0,  # No difference
    (lstm_predictions[["ptran"]] - abc_ptran) / abc_ptran * 100,
    (lstm_predictions[["R0"]] - abc_R0) / abc_R0 * 100
  )
)

knitr::kable(comparison_df, digits = 4, 
             caption = "Parameter Estimates: BiLSTM vs ABC (Recovery rate fixed at 1/7)")
```

### Parameter Uncertainty Analysis
```{r uncertainty-analysis}
# Calculate R0 statistics from posterior
# Note: posterior now has only 2 columns (contact_rate, transmission_prob)
R0_samples_calc <- (posterior[, 1] * posterior[, 2]) / abc_recov
abc_R0_lower <- quantile(R0_samples_calc, 0.025)
abc_R0_upper <- quantile(R0_samples_calc, 0.975)
abc_R0_sd <- sd(R0_samples_calc)

# Calculate SD for calibrated parameters
abc_sd <- apply(posterior, 2, sd)

# Create summary table with uncertainties
uncertainty_df <- data.frame(
  Parameter = c("Contact Rate", "Recovery Rate", "Transmission Prob", "R0"),
  ABC_Median = c(
    abc_crate, 
    abc_recov, 
    abc_ptran, 
    abc_R0
  ),
  ABC_Lower = c(
    abc_crate_lower, 
    abc_recov,  # No CI - fixed value
    abc_ptran_lower, 
    abc_R0_lower
  ),
  ABC_Upper = c(
    abc_crate_upper, 
    abc_recov,  # No CI - fixed value
    abc_ptran_upper, 
    abc_R0_upper
  ),
  ABC_SD = c(
    abc_sd[1],
    0,  # No SD - fixed value
    abc_sd[2],
    abc_R0_sd
  ),
  BiLSTM_Point = c(
    lstm_predictions[["crate"]],
    recov,
    lstm_predictions[["ptran"]],
    lstm_predictions[["R0"]]
  ),
  Status = c("Calibrated", "Fixed", "Calibrated", "Derived")
)

knitr::kable(uncertainty_df, digits = 4,
             caption = "Parameter Uncertainty: ABC provides posterior distributions for calibrated parameters")
```

### Combined Visualization
```{r combined-plot, fig.width=12, fig.height=8}
# Prepare data
plot_df <- data.frame(
  Date = covid_data$Date,
  Observed = incidence,
  BiLSTM_median = bilstm_quantiles$median,
  BiLSTM_lower = bilstm_quantiles$lower_ci,
  BiLSTM_upper = bilstm_quantiles$upper_ci,
  ABC_median = abc_quantiles$median,
  ABC_lower = abc_quantiles$lower_ci,
  ABC_upper = abc_quantiles$upper_ci
)

# Main comparison plot with distinct colors and boundary lines
p_comparison <- ggplot(plot_df, aes(x = Date)) +
  # ABC CI ribbon (bright blue)
  geom_ribbon(
    aes(ymin = ABC_lower, ymax = ABC_upper, fill = "ABC 95% CI"),
    alpha = 0.5
  ) +
  # BiLSTM CI ribbon (bright coral/red)
  geom_ribbon(
    aes(ymin = BiLSTM_lower, ymax = BiLSTM_upper, fill = "BiLSTM 95% CI"),
    alpha = 0.5
  ) +
  # ABC boundary lines (dashed blue)
  geom_line(
    aes(y = ABC_lower),
    color = "#2E86DE",
    linetype = "dashed",
    linewidth = 0.8
  ) +
  geom_line(
    aes(y = ABC_upper),
    color = "#2E86DE",
    linetype = "dashed",
    linewidth = 0.8
  ) +
  # BiLSTM boundary lines (dashed red)
  geom_line(
    aes(y = BiLSTM_lower),
    color = "#FF6B6B",
    linetype = "dashed",
    linewidth = 0.8
  ) +
  geom_line(
    aes(y = BiLSTM_upper),
    color = "#FF6B6B",
    linetype = "dashed",
    linewidth = 0.8
  ) +
  # Observed data (thick black line and points)
  geom_line(
    aes(y = Observed, color = "Observed"),
    linewidth = 1.5
  ) +
  geom_point(
    aes(y = Observed, color = "Observed"),
    size = 3
  ) +
  scale_color_manual(
    name = "",
    values = c("Observed" = "black"),
    labels = c("Observed Data")
  ) +
  scale_fill_manual(
    name = "",
    values = c(
      "ABC 95% CI" = "#2E86DE",      # Bright blue
      "BiLSTM 95% CI" = "#FF6B6B"    # Bright coral/red
    ),
    labels = c("ABC 95% CI", "BiLSTM 95% CI")
  ) +
  labs(
    title = "Comparison: BiLSTM vs ABC Calibration",
    subtitle = paste0(
      "BiLSTM: ", round(bilstm_total_mins, 1), " min | ",
      "ABC: ", round(abc_time_minutes, 1), " min (pre-computed) | ",
      "Speedup: ", round(speedup, 1), "x | ",
      "Recovery rate fixed at ", recov
    ),
    x = "Date",
    y = "Daily Infected Count"
  ) +
  theme_minimal(base_size = 14) +
  theme(
    legend.position = "bottom",
    plot.title = element_text(size = 16, face = "bold", hjust = 0.5),
    plot.subtitle = element_text(hjust = 0.5, size = 11),
    legend.box = "vertical",
    panel.grid.minor = element_blank()
  ) +
  guides(
    color = guide_legend(order = 1),
    fill = guide_legend(order = 2)
  )

print(p_comparison)
```

### Model Performance
```{r performance-metrics}
# BiLSTM metrics
bilstm_coverage <- plot_df %>%
  mutate(in_ci = Observed >= BiLSTM_lower & Observed <= BiLSTM_upper) %>%
  summarize(coverage = mean(in_ci) * 100) %>%
  pull(coverage)

bilstm_rmse <- sqrt(mean((plot_df$BiLSTM_median - plot_df$Observed)^2))
bilstm_mae <- mean(abs(plot_df$BiLSTM_median - plot_df$Observed))

# ABC metrics
abc_coverage <- plot_df %>%
  mutate(in_ci = Observed >= ABC_lower & Observed <= ABC_upper) %>%
  summarize(coverage = mean(in_ci) * 100) %>%
  pull(coverage)

abc_rmse <- sqrt(mean((plot_df$ABC_median - plot_df$Observed)^2))
abc_mae <- mean(abs(plot_df$ABC_median - plot_df$Observed))

# Create comparison table
metrics_df <- data.frame(
  Metric = c("95% CI Coverage (%)", "RMSE", "MAE", "Calibration Time (min)", "Parameters Calibrated"),
  BiLSTM = c(
    round(bilstm_coverage, 1),
    round(bilstm_rmse, 2),
    round(bilstm_mae, 2),
    round(bilstm_total_mins, 2),
    "2 (contact, trans)"
  ),
  ABC = c(
    round(abc_coverage, 1),
    round(abc_rmse, 2),
    round(abc_mae, 2),
    round(abc_time_minutes, 2),
    "2 (contact, trans)"
  )
)

knitr::kable(metrics_df, 
             caption = "Model Performance Comparison (Recovery rate = 1/7 fixed for both)")
```

## Summary

This analysis demonstrates that:

1.  **BiLSTM is significantly faster** (\~`r round(speedup, 0)`x speedup) than ABC calibration
2.  **Both methods produce comparable accuracy** in terms of RMSE and MAE
3.  **Uncertainty quantification** is similar between methods, with \~`r round(mean(c(bilstm_coverage, abc_coverage)), 0)`% coverage
4.  **BiLSTM provides instant predictions** making it ideal for real-time applications
5.  **ABC offers full Bayesian inference** with credible intervals for calibrated parameters
6.  **Both methods leverage domain knowledge** by fixing the recovery rate at 1/7 (7-day infectious period)

### Key Advantages

**BiLSTM:**
- Extremely fast calibration (\<1 minute)
- Point estimates for rapid decision-making
- Suitable for operational deployment
- Real-time epidemic monitoring
- Calibrates 2 parameters efficiently

**ABC:**
- Complete posterior distributions for calibrated parameters
- Full uncertainty quantification
- Credible intervals for contact rate and transmission probability
- Ideal for research and policy analysis
- Better for understanding parameter uncertainty
- Reduced dimensionality (2 vs 3 parameters) improves convergence

### Methodological Note: Fixed Recovery Rate

Both BiLSTM and ABC use the **known recovery rate** (1/7) as a fixed parameter:

- **Domain Knowledge**: COVID-19 has a well-established ~7-day infectious period
- **Fair Comparison**: Both methods use identical domain knowledge
- **Reduced Uncertainty**: Fixing one parameter improves estimation of the remaining two
- **Faster Convergence**: ABC with 2 parameters converges more quickly than with 3
- **Focus on Key Parameters**: Contact rate and transmission probability are the primary drivers of epidemic dynamics

### Recommendation

The choice between methods depends on the use case:
- **BiLSTM** for speed and operational deployment
- **ABC** for comprehensive uncertainty quantification and parameter inference
- **Hybrid approach**: Use BiLSTM for initial screening, ABC for detailed analysis

### Package Data Note

This vignette uses pre-computed ABC calibration results stored in `abc_calibration_params`. The ABC calibration was performed using LFMCMC with `r n_samples_abc` samples (`r burnin_abc` burn-in), calibrating only 2 parameters (contact rate and transmission probability), and took approximately `r round(abc_time_minutes, 1)` minutes to complete.

To re-run the ABC calibration yourself, see the script in `data-raw/process_covid_calibration.R`.
