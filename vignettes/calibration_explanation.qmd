...
For transparency (not required for users), `show_preprocessing()` demonstrates the percentage-change transform applied internally.
...
trajectory_summary <- trajectory_comparison %>%
  summarize(
...
## Optional: Model Lifecycle Helpers

These are convenience helpers; ordinary users don’t need them.

```{r}
status <- check_model_status()
status

# When done, free memory (optional)
# cleanup_model()
```
...
results_list <- list(
  ...
  trajectory_bias      = trajectory_summary,
...
```{r, eval=FALSE}
# Have exactly 61 consecutive daily case counts (day 0..60)
my_incidence <- c(...)          # numeric length 61
population   <- 10000
recovery     <- 1/7             # 7-day infectious period

# One line:
params <- calibrate_sir(my_incidence, population, recovery)
print(params)  # named vector: ptran, crate, R0
```

### Notes

* If the required Python packages are missing, `calibrate_sir()` will attempt to set up a private Python environment and install them (internet required). If installation isn’t permitted by your system, the function will return a clear error with instructions.
* Model assets are bundled with the package; no paths need to be provided for common use. and
