...
recov <- 1 / 7  # Recovery rate (KNOWN: 7-day infectious period)
model_ndays <- 60  # Simulation days
model_seed <- 122

cat("\nUsing KNOWN recovery rate:", recov, "(7-day infectious period)\n")
cat("Calibrating only: contact_rate and transmission_prob\n\n")
...
# CHANGED: Only 2 parameters now (contact_rate and transmission_prob)
simulation_fun <- function(params, lfmcmc_obj, observed_data_info) {
  sim_params <- c(
    ...
    observed_data_info$recovery_rate, # recovery_rate (FIXED/KNOWN)
    params[2]                         # transmission_prob
  )
...
# CHANGED: Proposal function for only 2 parameters
proposal_fun <- function(old_params, lfmcmc_obj) {
  ...
  # Transmission prob: use logit transform (bounded [0,1])
  new_ptran <- plogis(qlogis(old_params[2]) + rnorm(1, sd = 0.025))

  return(c(new_crate, new_ptran))
}

# Weighted kernel function to prevent any stat from dominating
...
# CHANGED: Include known recovery rate in observed_data_info
observed_data_info <- list(
  n = N,
  preval = initial_prevalence,
  recovery_rate = recov  # FIXED/KNOWN parameter
)
...
# Apply summary function to observed data before setting it
...
# CHANGED: Only 2 initial parameters now
init_params <- c(5, 0.0571)  # contact_rate, transmission_prob
...
cat("Calibrating 2 parameters: contact_rate, transmission_prob\n")
cat("Fixed parameter: recovery_rate =", recov, "\n")
...
# CHANGED: Extract only 2 parameters
abc_crate <- abc_median[1]
abc_ptran <- abc_median[2]
abc_recov <- recov  # Use the KNOWN value
...
cat("Recovery Rate:", round(abc_recov, 4), "(FIXED/KNOWN)\n")
cat("Transmission Prob:", round(abc_ptran, 4),
    "[", round(abc_lower[2], 4), ",", round(abc_upper[2], 4), "]\n")
cat("R0:", round(abc_R0, 4), "\n")

# Save calibration results -------------------------------------------------
# CHANGED: Variable name is now abc_calibration_params2
abc_calibration_params2 <- list(
  contact_rate = abc_crate,
  recovery_rate = abc_recov,  # Known/fixed value
  transmission_prob = abc_ptran,
  R0 = abc_R0,
  contact_rate_ci = c(lower = abc_lower[1], upper = abc_upper[1]),
  recovery_rate_ci = c(lower = recov, upper = recov),  # No CI since it's fixed
  transmission_prob_ci = c(lower = abc_lower[2], upper = abc_upper[2]),
  ...
  recovery_rate_fixed = TRUE,  # Flag indicating recovery rate was fixed
  n_calibrated_params = 2      # Only 2 parameters were calibrated
)

# Save to data folder
usethis::use_data(abc_calibration_params2, overwrite = TRUE)

cat("\n✓ Saved abc_calibration_params2 to data/abc_calibration_params2.rda\n")
cat("  Calibration time:", round(abc_calibration_time, 2), "seconds\n")
cat("  Acceptance rate:", round(acceptance_rate, 2), "%\n")
cat("  Parameters calibrated: 2 (contact_rate, transmission_prob)\n")
cat("  Parameters fixed: 1 (recovery_rate = ", recov, ")\n", sep = "")
